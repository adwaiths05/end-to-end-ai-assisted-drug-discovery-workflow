...
export default function AnalysisPage() {
  ...
  const { token } = useAuth();
  const [activeTab, setActiveTab] = useState<'rankings' | 'interactions' | '3d' | 'comparison' | 'history'>('rankings');
  const [selected3DCompoundId, setSelected3DCompoundId] = useState<string>('');
  ...
  const combinedResults = screeningSession.results.map(screening => {
    const docking = dockingJob.results.find(d => d.smiles === screening.smiles);
    return { ...screening, bindingAffinity: docking?.affinity };
  }).sort((a, b) => (a.bindingAffinity ?? 1) - (b.bindingAffinity ?? -1));
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="rounded-lg border border-border bg-card overflow-hidden">
          <div className="flex gap-0 border-b border-border overflow-x-auto">
            {['rankings', '3d', 'interactions', 'comparison', 'history'].map(id => (
          ...
          <div className="p-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <select
                    ...
                    value={selected3DCompoundId || combinedResults[0]?.compoundId || ''}
                    onChange={(e) => setSelected3DCompoundId(e.target.value)}
                  >
                ...
                {(() => {
                  const activeCompoundId = selected3DCompoundId || combinedResults[0]?.compoundId || 'unknown';
                  // Vina files are saved by their original index in the docking job
                  const originalIndex = dockingJob.results.findIndex(r => r.compoundId === activeCompoundId);
                  const fileName = originalIndex >= 0 ? `ligand_${originalIndex}_out.pdbqt` : 'ligand_0_out.pdbqt';
                  
                  // Component key ensures it entirely remounts when the filename changes
                  return <MolecularViewer3D key={fileName} compoundId={activeCompoundId} fileName={fileName} />;
                })()}
              </div>
            )}
            ...
            {activeTab === 'rankings' && (
              <table className="w-full text-sm">
                <thead><tr className="border-b border-border"><th className="px-4 py-3 text-left">Rank</th><th className="px-4 py-3 text-left">Compound</th><th className="px-4 py-3 text-right">Consensus</th><th className="px-4 py-3 text-right">Affinity</th></tr></thead>
                <tbody>
                  {combinedResults.slice(0, 15).map((r, idx) => (
                    <tr key={idx} className="border-b border-border hover:bg-muted/50"><td className="px-4 py-3">{idx + 1}</td><td className="px-4 py-3">{r.compoundId}</td><td className="px-4 py-3 text-right">{r.predicted_pic50?.toFixed(3)}</td><td className="px-4 py-3 text-right text-primary font-semibold">{r.bindingAffinity?.toFixed(2)}</td></tr>
                  ))}
                </tbody>
              </table>
            )}
              ...
              <div className="space-y-4">
                <ResponsiveContainer width="100%" height={300}>
                  <ScatterChart>
                    ...
                    <XAxis dataKey="x" name="Consensus" type="number" stroke="var(--color-muted-foreground)" />
                    <YAxis dataKey="y" name="Affinity" type="number" stroke="var(--color-muted-foreground)" />
            ...
            {activeTab === 'history' && (
              <div className="space-y-6">
                <h3 className="text-lg font-semibold">Model Performance Trend</h3>
                <ResponsiveContainer width="100%" height={250}>
                  <LineChart data={historyData}>
                    ...
                    <Line type="monotone" dataKey="avg_pic50" stroke="var(--color-primary)" name="Avg pIC50" />
                  </LineChart>
                </ResponsiveContainer>
                <table className="w-full text-sm mt-8">
                  <thead><tr className="border-b border-border"><th className="text-left pb-2">Date</th><th className="text-left pb-2">Ensemble</th><th className="text-right pb-2">Compounds</th><th className="text-right pb-2">Avg pIC50</th></tr></thead>
                  <tbody>
                    {historyData.map(run => (
                      <tr key={run.id} className="border-b border-border/50"><td className="py-2">{new Date(run.created_at).toLocaleDateString()}</td><td className="py-2 capitalize">{run.ensemble}</td><td className="text-right py-2">{run.compounds}</td><td className="text-right py-2">{run.avg_pic50?.toFixed(2)}</td></tr>
                    ))}
                  </tbody>
                </table>
              </div>
